...
class TemperatureController():
	def __init__(self, fig, **kw):
		self.reward=0
		self.fig = fig
		self.ani = animation.FuncAnimation(self.fig, self.animate, fargs=(xs, ys), interval=1000)
		self.fig.show()
	...
	def animate(self, i, xs, ys):
		...
		internalTemperature, kilnTemperature, kilnOn, kilnHeartbeat = self.readLineFromArduino()
